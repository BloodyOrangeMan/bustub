...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(
        fmt::format("Incorrect number of arguments for function {}. Expected 1, but got {}.", func_name, args.size()));
  }

  // Check the function name and return the appropriate expression.
  if (func_name == "lower") {
